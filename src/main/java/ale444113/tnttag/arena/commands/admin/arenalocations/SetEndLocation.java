...
public class SetEndLocation implements CommandExecutor {
    private TNTTag plugin;

    public SetEndLocation(TNTTag plugin){
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        ConfigurationSection startLocation = config.getConfigurationSection(args[0]).getConfigurationSection("endLocation");
        ...
        Arena.arenaEndLocations.put(args[0], playerLocation);

        player.sendMessage(plugin.name+ChatColor.AQUA+args[0]+ChatColor.GREEN+" end location updated");
        return true;
    }
}
